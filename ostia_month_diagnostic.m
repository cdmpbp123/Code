...
% daily input path
daily_input_path = [basedir, './Result/ostia/',domain_name,'/daily']; 
result_path = [basedir,'./Result/ostia/',domain_name,'/climatology/'];mkdir(result_path)
result_fn = [result_path,'/ostia_front_monthly_climatology_',smooth_type,'_',num2str(yy1),'to',num2str(yy2),'.nc'];
%
fig_path = [basedir,'./Fig/ostia/',domain_name,'/climatology/'];mkdir(fig_path);

if ~exist(result_fn)
    % get OSTIA grd from test file
    iy = 2017; im = 1; id = 1;
    day_string = [num2str(iy),num2str(im,'%2.2d'),num2str(id,'%2.2d')];
    test_fn = [daily_input_path,'/',num2str(yy1), '/detected_front_', day_string, '.mat'];
    if ~exist(test_fn)
        error('can not find the file, choose another MAT file to read grd file')
    end
