% front occurence frequency in raw grid for OSTIA daily 
...
result_path = [basedir, './Result/ostia/', domain_name, '/climatology/'];
% input file
clim_result_fn = [result_path, '/ostia_front_monthly_climatology_', smooth_type, '_', num2str(yy1), 'to', num2str(yy2), '.nc'];
fig_path = [basedir, './Fig/ostia/', domain_name, '/climatology/']; mkdir(fig_path);
result_fn = [result_path, '/ostia_front_frequency_map_', smooth_type, '_', num2str(yy1), 'to', num2str(yy2), '.nc'];
%
% concat daily data
tgrad_daily = [];
bw_daily =[];
...
high_thresh_daily = [];
for yy = yy1:yy2
    daily_result_fn = [result_path, '/ostia_tgrad_daily_',smooth_type, '_year',num2str(yy),'.nc'];
...
tt = toc;
save('ostia_front_freq_map.mat','frontarea_freq_map','frontline_freq_map')
if 1 == 1
    delete(result_fn)
    ...
    ncwriteatt(result_fn, '/', 'data_source', 'OSTIA daily output')
