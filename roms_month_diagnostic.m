...
fill_value = 0;
% detect parameter
flen_crit4diag = 50e3; % set new length criterion for diagnostic
...
% daily input path
daily_input_path = [basedir, './Result/roms/',domain_name,'/daily']; 
result_path = [basedir,'./Result/roms/',domain_name,'/climatology/'];mkdir(result_path)
result_fn = [result_path,'/roms_front_monthly_climatology_',smooth_type,'_',num2str(yy1),'to',num2str(yy2),'.nc'];
%
...
if ~exist(result_fn)
    ...
    for im = 1:nt
        ...
        % auto-threshold
        thresh_fig_name = [fig_path,'/roms_tgrad_threshold_month_',num2str(im,'%2.2d'),'_',smooth_type,'_',num2str(yy1),'to',num2str(yy2),'.jpg'];
...
title('climatology front length diagnostic for ROMS')
ylabel('km')
set(gca,'YLim',[80 200])
...
title('climatology front strength diagnostic for ROMS')
ylabel('\circC/km')
...
title('climatology front width diagnostic for ROMS')
ylabel('km')
set(gca,'YLim',[30 80])
set(gca,'XTickLabel',xx)
...
title('climatology front area diagnostic for ROMS')
ylabel('km2')
...
title('climatology front number diagnostic for ROMS')
ylabel('#')
set(gca,'YLim',[60 200])
set(gca,'XTickLabel',xx)
