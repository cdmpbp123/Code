...
% make daily validation with observation OSTIA SST
close all
clear all
clc
%
...
fig_show = 'off';
bin_ratio_test = 0; % set test for front ratio in each bin
for iy = yy1:yy2
    ...
    for iday = 1:ndays
        ...
        %
        [area_bias(iday), area_TS(iday), area_FAR(iday), area_MR(iday), area_FA(iday), area_DR(iday)] = front_skill_score(roms_area, ostia_area);
        clear roms_area ostia_area 
    ...
    hold on
    plot(xlabel,area_FAR,'-k','LineWidth',LineWidth)
    hold on
    plot(xlabel,area_MR,'--k','LineWidth',LineWidth)
    hold on
    plot(xlabel,area_FA,'--r','LineWidth',LineWidth)
    hold on
    legend('bias','TS','False Alarm Rate','Miss Rate','Forecast Accuracy','Location','best')
    title(num2str(iy))
    ylabel('scores')
    grid on
    set(gca,'XTick',xtick)
    set(gca,'XTickLabel',xx)
    export_fig([scores_fig_path,'front_area_scores_bin_',num2str(bin_resolution),'degree_',num2str(iy),'.png'],'-png','-r200');
